...
export class AgentManager {
  ...
  triggerVortex() {
    ...
    // Save all agent positions so we can return them after
  ...
  private createVortexVisuals() {
    ...
    // Dark overlay — gets darker over time
    ...
    // === PHASE 1: Opening (0-3s) — rings explode outward ===

    // 8 spinning ring layers — thick, bright, aggressive
    ...
    for (let i = 0; i < 8; i++) {
      ...
      // Accelerating rotation — gets faster over time
      const baseDuration = 4000 - i * 400;
      this.scene.tweens.add({
        targets: ring,
        angle: i % 2 === 0 ? 360 : -360,
      ...
      // Second tween that accelerates rings mid-way
    ...
    // Central core — massive, pulsing white-hot
    ...
    // Lightning — gets more frequent and branching over time
    const boltEvent = this.scene.time.addEvent({
      ...
      callback: () => {
        ...
        const boltCount = 1 + Math.floor(fury * 3); // 1→4 simultaneous bolts
    ...
    // Debris / chunks pulled from edges of screen
    const debrisEvent = this.scene.time.addEvent({
      ...
      callback: () => {
        ...
        const elapsed = this.scene.time.now - this.vortexStartTime;
        if (elapsed < 2000) return; // start after 2s
    ...
    // Periodic screen shake — escalating
    ...
    // === PHASE ESCALATION: color shifts and screen flash at milestones ===
    ...
    // Countdown text in final 5 seconds
  ...
  private updateVortexSwirl(time: number, elapsed: number) {
    ...
    // Progressive acceleration curve (15s):
    // 0-1.5s: slow suck-in (0→0.5)
    // 1.5-4s: accelerating (0.5→1.5)
    // 4-9s: fast (1.5→3.0)
    // 9-13s: furious (3.0→5.5)
    // 13-15s: decelerate for ending (5.5→0.5)
    ...
    // Radius shrinks as it gets faster — agents pulled tighter
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Multi-layer orbit — agents weave in and out
      const layer = i % 4;
      ...
      // Lerp speed increases with speedMult for snappier movement
  ...
  private endVortex() {
    ...
    // Implosion then explosion — suck in, then burst
    ...
    // Massive particle explosion — multiple rings
    ...
    for (let i = 0; i < 4; i++) {
      this.scene.time.delayedCall(i * 150, () => {
        ...
        this.scene.tweens.add({
          targets: wave,
          scaleX: 15 + i * 3, scaleY: 10 + i * 2, alpha: 0,
    ...
    // Fling agents outward dramatically, then return
    for (const [id, agent] of this.agents) {
      const saved = this.preVortexPositions.get(id);
      if (!saved) continue;

      // Fling outward from center first
      const angle = Math.atan2(agent.y - cy, agent.x - cx);
      const flingDist = Phaser.Math.Between(80, 150);
      const flingX = agent.x + Math.cos(angle) * flingDist;
      const flingY = agent.y + Math.sin(angle) * flingDist;

      agent.setPosition(
        Phaser.Math.Clamp(flingX, 20, 780),
        Phaser.Math.Clamp(flingY, 50, 460),
      );

      // Then walk home after a dazed pause
